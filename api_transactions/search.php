...
<?php echo $LANG['search_here_id_email_amount_phone']; ?>
...
$totalQuery = $conn->query("SELECT id FROM api_transaction WHERE (id='$q' OR status ='$q' OR phone ='$q' OR amount ='$q' OR email ='$q' ) AND  user='$loginUser'")->num_rows;
$page->searchForm($action,$q);
$pageData = $page->getData($currentPage,$totalQuery);
$start = $pageData["start"];
$stop = $pageData["stop"];

$sql = "SELECT service_id,id,amount,status FROM api_transaction WHERE (id='$q' OR status ='$q' OR phone ='$q' OR amount ='$q' OR email ='$q' ) AND user='$loginUser' LIMIT $start,$stop";
