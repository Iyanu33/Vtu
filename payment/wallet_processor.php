...
if ($result->num_rows > 0) {
 ...
 if($payment["settled"]==1){
  ...
  $finalBalance = $payment["amount"]+$balance; 
  $conn->query("UPDATE users SET credit='$finalBalance' WHERE id='{$payment["owner"]}'");
}else{
    $errorFound = true;
    $error[]="transaction_failed_unable_to_verify_payment";
}

$conn->query("UPDATE payment SET settled='1', ini_balance='$balance', final_balance='$finalBalance' WHERE id='{$payment['id']}'");
