...
<?php 
$userInfo = userInfo($loginUser,$conn);
$GLOBALS["user"] = $userInfo  = json_decode($userInfo,true);
?>
<?php 
if(!empty($_GET["id"])){
          include "recharge_processor.php";
          $transactionId = xcape($conn, $_GET["id"]);
           $transaction = payService($conn, $transactionId,"",true);
           if($transaction===true){
              alertSuccess($LANG["transaction_successful"]); 
           }else{
               alertDanger($LANG["transaction_failed"]);
           }
}
?>
